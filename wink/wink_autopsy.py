...
class WinkIngestModuleFactory(IngestModuleFactoryAdapter):
    moduleName = "Wink Analysis"
    ...
    def getDefaultIngestJobSettings(self):
        return WinkIngestModuleSettings()
    ...
    def getIngestJobSettingsPanel(self, settings):
        if not isinstance(settings, WinkIngestModuleSettings):
            raise IllegalArgumentException(
                "Expected settings argument to be instance of WinkIngestModuleSettings")
        self.settings = settings
        return WinkIngestModuleSettingsPanel(self.settings)

    def isDataSourceIngestModuleFactory(self):
        return True

    def createDataSourceIngestModule(self, ingestOptions):
        return WinkIngestModule(self.settings)


# Data Source-level ingest module.  One gets created per data source.
class WinkIngestModule(DataSourceIngestModule):
    _logger = Logger.getLogger(WinkIngestModuleFactory.moduleName)
    ...
    def process(self, dataSource, progressBar):
        ...
        if self.local_settings.get_parse_settings():
            ...
            for file in setting_file:
                ...
                att = BlackboardAttribute(BlackboardAttribute.ATTRIBUTE_TYPE.TSK_SET_NAME,
                                          WinkIngestModuleFactory.moduleName, "Wink")
                ...
                wink_settings = minidom.parse(lcl_setting_path)
                tags = wink_settings.getElementsByTagName("string")
                wink_logins = {}
                for tag in tags:
                    if tag.getAttribute('name') == "email":
                        wink_logins['username'] = str(tag.firstChild.data)
                    elif tag.getAttribute('name') == "token":
                        wink_logins['token'] = str(tag.firstChild.data).replace("\n", "")
                    elif tag.getAttribute('name') == "userId":
                        wink_logins['user_id'] = str(tag.firstChild.data)
                ...
                IngestServices.getInstance().fireModuleDataEvent(
                    ModuleDataEvent(WinkIngestModuleFactory.moduleName, art_type, None))
        ...
        message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                              "Wink Analysis", "Found %d files" % file_count)
...
class WinkIngestModuleSettings(IngestModuleIngestJobSettings):
    serialVersionUID = 1L
    ...
    def __str__(self):
        return "Wink Parser - Settings: Parse_DB = {}, Parse_Settings = {}".format(
            self.parse_cache, self.parse_settings)


# UI that is shown to user for each ingest job so they can configure the job.
# TODO: Rename this
class WinkIngestModuleSettingsPanel(IngestModuleIngestJobSettingsPanel):
