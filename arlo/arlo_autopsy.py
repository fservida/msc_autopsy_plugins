...
class ArloIngestModuleFactory(IngestModuleFactoryAdapter):
    moduleName = "Arlo Analysis"
    ...
    def getDefaultIngestJobSettings(self):
        return ArloIngestModuleSettings()
    ...
    def getIngestJobSettingsPanel(self, settings):
        if not isinstance(settings, ArloIngestModuleSettings):
            raise IllegalArgumentException(
                "Expected settings argument to be instance of ArloIngestModuleSettings")
        self.settings = settings
        return ArloIngestModuleSettingsPanel(self.settings)

    def isDataSourceIngestModuleFactory(self):
        return True

    def createDataSourceIngestModule(self, ingestOptions):
        return ArloIngestModule(self.settings)


# Data Source-level ingest module.  One gets created per data source.
class ArloIngestModule(DataSourceIngestModule):
    _logger = Logger.getLogger(ArloIngestModuleFactory.moduleName)
    ...
    def process(self, dataSource, progressBar):
            ...
            for file in setting_file:
                ...
                for tag in tags:
                    if tag.getAttribute('name') == "email":
                        ...
                        arlo_logins['token'] = str(tag.firstChild.data).replace("\n", "")
                ...
                att_login_username = BlackboardAttribute(att_login_username_id,
                                                         ArloIngestModuleFactory.moduleName, arlo_logins['username'])
                att_login_secret = BlackboardAttribute(att_login_secret_id,
                                                       ArloIngestModuleFactory.moduleName,
                                                       arlo_logins['token'])
                ...
                IngestServices.getInstance().fireModuleDataEvent(
                    ModuleDataEvent(ArloIngestModuleFactory.moduleName, art_type, None))
        ...
        message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                              "Arlo Analysis", "Found %d files" % file_count)
...
class ArloIngestModuleSettings(IngestModuleIngestJobSettings):
    serialVersionUID = 1L
    ...
    def __str__(self):
        return "Arlo Parser - Settings: Parse_DB = {}, Parse_Settings = {}".format(
